...
namespace Blog.Controllers
{
    ...
    public class ArticleModelController : Controller
    {
        ...
        public async Task<IActionResult> MyBlogs(int page =1)
        {
            ...
            int pageSize = 10;
            IQueryable<indexArticleViewModel> source = _article.GetBlogByAvtorMinVersion(User.Identity.Name);
        ...
        public async Task<IActionResult> Find(FindViewModel model)
        {
            ...
            var blogs = _article.GetAllBlogMinVersion();
        ...
        public async Task<IActionResult> Index(int page=1)
        {
            ...
            int pageSize = 10 ;
            IQueryable<indexArticleViewModel> source = _article.GetAllBlogMinVersion(); 
            var count = await source.CountAsync();
            var items = await source.Skip((page - 1) * pageSize).Take(pageSize).ToListAsync();
        ...
        [Authorize]
        public async Task<ActionResult> Create([Bind("Id,Tags,Description,ShortDesk,Title,HelloImage,AvtorName,CategoryId,DescriptionNotParse")]ArticleModel model)
        {
            //View Bags
        ...
        [ValidateAntiForgeryToken]
        [Authorize]
        public async Task<ActionResult> Edit(int id,[Bind("Id,Tags,BlogName,Description,ShortDesk,Title,HelloImage,AvtorName,CategoryId,DescriptionNotParse")]ArticleModel model,string OldName)
        {
            var item = _category.AllCategories;
        ...
        [HttpPost]
        public void SetSesionValues(string width)
        {
           // int Width;
            if (!string.IsNullOrEmpty(width))
            {
                ...
                {
                    HttpContext.Session.SetString("width","1500" );
                }
            }
        }
    }
